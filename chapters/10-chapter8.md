...
###  Prompt Engineering
Construct prompts to guide the LLM towards a specific type of answer and/or persona. For example, you can prompt the LLM to act as a "healthcare physician who ansers medical queries based on medical knowledge from trusted sources". The motivation is to elicit specific behaviors and desired outputs from the LLM. There are different types of prompting:

1. zero-shot learning - expect response from LLM without any example
...
1. grounding - reduce hallucinations by restricting the answer to the (query-related) data from the database
